...
- 分清 Agent 和普通对话系统的差别
- 理解 Agent 的价值不在“会聊天”，而在“能完成目标”
- 知道规划、工具、记忆、反思分别承担什么角色
- 对 Agent 系统的边界和风险有更现实的判断

## 先纠正一个常见误解

Agent 不等于“更会聊天的 LLM”。

更准确地说，Agent 是一种围绕目标执行构建出来的系统，它通常具备：
...
所以 Agent 重点不是文本生成本身，而是“围绕任务完成的循环”。

## 1. 什么才算 Agent

### 1.1 一个更实用的定义

Agent 是一种围绕目标运行的系统，它会把模型能力和外部工具、环境反馈、状态管理结合起来，而不是只做一次性问答。
...
很多实际可用 Agent，本质上就是“LLM + 明确工具集 + 可控执行策略”。这类系统往往比空泛追求自主性更有价值。
...
很多人学 Agent 时会把注意力放在框架名字上，但工程里更重要的是这些问题：
