...
- 理解 RAG 真正解决的是什么问题
- 分清检索、生成、重排、引用这几个模块的角色
- 避免把所有“知识问答”都简单理解成向量检索 + LLM
- 能够用系统视角判断一个 RAG 应用为什么效果不好

## 先说结论：RAG 不是外挂，而是补边界

RAG（Retrieval-Augmented Generation）最重要的价值，不是“让模型变强”，而是补足模型参数内知识的边界，尤其是：
...
所以 RAG 更适合被理解成一个系统设计方案，而不是模型训练技术。
...
那就不能只靠 RAG，往往还需要工作流、工具调用、Agent 或更强的应用编排。
...
很多 RAG 效果差，并不是模型不行，而是切块策略错了。分块过大、过小、断句错误、上下文丢失，都会直接影响检索质量。
...
如果上下文喂得不清楚，即使检索对了，回答也可能依旧发散、漏答或编造。
